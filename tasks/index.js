...
task("migrate", "Migrates liquidity from Uniswap to PogoSwap")
  .addOptionalParam(
    "a",
    "Token A",
...
task("bar:enter", "PogoBar enter")
  ...
  .setAction(async function (
    ...
    const bar = await ethers.getContract("PogoBar");

    await run("erc20:approve", { token: Pogo.address, spender: bar.address });

    await (await bar.connect(await getNamedSigner("dev")).enter(amount)).wait();
  });

task("bar:leave", "PogoBar leave")
  .addParam("amount", "Amount")
  .setAction(async function (
    { amount },
    { ethers: { getNamedSigner } },
...
task("maker:serve", "PogoBar serve")
  .addParam("a", "Token A")
  .addParam("b", "Token B")
  .setAction(async function (
    { a, b },
    ...
    const maker = await ethers.getContract("PogoMaker");
